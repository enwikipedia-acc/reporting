...
function writeCreateData($requestData)
{
    ...
    foreach (array_keys($criteria) as $key) {
        ...
        $criteria[$key]['sql'] = fopen($key.'.sql', 'w');

        $criteria[$key]['hasitems'] = false;

        writeFileHeader($criteria[$key]['html']);
        fwrite($criteria[$key]['html'], '<table>');
        fwrite($criteria[$key]['html'], '<tr><th>Request date</th><th>ID</th><th>Name</th><th>Email</th><th>Search</th><th>Reserve</th><th>comment</th></tr>');
    }
    ...
    foreach ($requestData as $id => $data) {
        if (count($data) === 0) {
            ...
            if($alternateSwitched) {
                foreach (array_keys($criteria) as $key) {
                    fwrite($criteria[$key]['sql'], '-- '.$data['date']."\n");
                }
            }
            ...
            foreach (array_keys($criteria) as $key) {
                if($includeThis[$key]) {
                    ...
                    fwrite($criteria[$key]['sql'], 'INSERT INTO jobqueue (task, user, request, parameters) VALUES (\'Waca\\\\Background\\\\Task\\\\RemoteCreationTask\', 1, 1, \'{"requestid" : '.$id.'}\');'."\n");
                }
            }
        }
